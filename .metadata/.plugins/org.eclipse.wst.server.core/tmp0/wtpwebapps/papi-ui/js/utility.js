function createDynamiTable(groupData, id) {
	...
	for (var i = 0; i < groupData.length; i++) {
		...
		for (var j = 0; j < col.length; j++) {
			...
			if (j == col.length - 1) {
				tabCell.innerHTML = '<div onclick=editGroup("'
						+ groupData[i]
						+ '")><i class="fa fa-pencil"></i></div> <a href="'
						+ groupData[i][col[j]]
...
function createChatList(chats, user_id, group_id, div_id) {
	...
	for (var k = 0; k < chats.length; k++) {
		...
		if (user_id == chats[k].user_id) {
			img_div.setAttribute("src", "images/img_avatar.png");
		} else {
			img_div.setAttribute("src", "images/img_avatar.png");
		}
		if (user_id == chats[k].user_id) {
...
function getChatList(groupId) {
	...
	//data : data //'{"name":"' + $("#pw_grp_name").val() + '"}'
		...
		//showSnackBar("Loading Messages...")
		createChatList(chatList, userDetails.user.id, groupId , "chatList")
		//scrollToBottom('messages-container','div')
